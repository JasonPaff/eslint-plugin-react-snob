...
/**
 * Checks if a node is a function-like value (function expression, arrow function, or React HOC call)
 */
function isFunctionLikeValue(node: TSESTree.Node | null | undefined): boolean {
  if (!node) return false;

  // Direct function expressions or arrow functions
  if (node.type === 'ArrowFunctionExpression' || node.type === 'FunctionExpression') {
    return true;
  }

  // Call expressions that might be React HOCs (forwardRef, memo, etc.)
  if (node.type === 'CallExpression') {
    // Check if it's a known React HOC
    if (node.callee.type === 'Identifier') {
      const hocNames = ['forwardRef', 'memo', 'lazy', 'observer'];
      if (hocNames.includes(node.callee.name)) {
        return true;
      }
    }

    // Recursively check nested calls (e.g., memo(forwardRef(...)))
    if (node.arguments.length > 0) {
      const firstArg = node.arguments[0];
      if (isFunctionLikeValue(firstArg)) {
        return true;
      }
    }
  }
...
export function isComponentFunction(node: TSESTree.Node): boolean {
  ...
  if (node.type === 'VariableDeclarator' && node.id.type === 'Identifier' && /^[A-Z]/.test(node.id.name)) {
    // For variable declarators, ensure the value is actually a function or function-like
    return isFunctionLikeValue(node.init);
  }
