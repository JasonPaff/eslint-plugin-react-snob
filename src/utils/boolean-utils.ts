...
export function isBooleanExpression(node: TSESTree.Expression): boolean {
  switch (node.type) {
    ...
    case 'LogicalExpression':
      ...
      // For || and &&, check if at least one operand is a non-boolean literal
      // If so, the expression doesn't necessarily return boolean
      ...
      // If both operands are boolean expressions, it returns boolean-like values
