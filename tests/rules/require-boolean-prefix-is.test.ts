...
// Test cases for logical OR with non-boolean types that should be ignored
const logicalOrNonBooleanCases = [
  createValidCase(`const value = someString || 'default';`),
  createValidCase(`const [activeTab, setActiveTab] = useState(refinements[0]?.agentId || '');`),
  createValidCase(`const name = user?.name || 'Anonymous';`),
  createValidCase(`const count = data?.count || 0;`),
];
